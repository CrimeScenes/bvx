...
-- Improved function to predict the target's future position based on velocity
local function predictPosition(targetPlayer)
    ...
    local velocity = humanoidRootPart.Velocity  -- Get the velocity of the target
    local predictedPosition = humanoidRootPart.Position + velocity * 0.1  -- Predict where the target will be in 0.1 seconds
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                -- Use the predicted position instead of the current position
